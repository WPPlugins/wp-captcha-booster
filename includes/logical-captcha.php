...
function captcha_booster_login_form() {
   ...
   if (isset($_SESSION["captch_booster_login"])) {
      unset($_SESSION["captch_booster_login"]);
   }
   echo "<p class=cptch_block>";
   if ("" != $captcha_booster_options["captcha_label_form"]) {
      echo "<label>" . $captcha_booster_options["captcha_label_form"] . "<span class=required > " . $captcha_booster_options["captcha_required_symbol"] . "</span></label><br />";
   }

   if (isset($_SESSION["captcha_booster_error"])) {
      echo "<br/><span style=\"color:red;\">" . $_SESSION["captcha_booster_error"] . "</span><br/>";
      unset($_SESSION["captcha_booster_error"]);
   }
   echo "<br/>";
...
function captcha_booster_login_check($user, $username, $password) {
   ...
   if (isset($_SESSION["captch_booster_login"]) && true === $_SESSION["captch_booster_login"]) {
      return $user;
   }
   ...
   if ((!isset($_REQUEST["ux_txt_captcha_input"]) || "" == esc_attr($_REQUEST["ux_txt_captcha_input"])) && isset($_REQUEST["loggedout"])) {
      $error = new WP_Error();
      $error->add("captcha_booster_error", "<strong>" . captcha_booster_logical_error . "</strong>: " . $error_data_array["for_captcha_empty_error"]);
      wp_clear_auth_cookie();
      return $error;
   }
   if (isset($_REQUEST["captcha_booster_result"]) && isset($_REQUEST["ux_txt_captcha_input"]) && isset($_REQUEST["captcha_booster_time"])) {
      if (0 == strcasecmp(trim(captcha_booster_decode($_REQUEST["captcha_booster_result"], $str_key, $_REQUEST["captcha_booster_time"])), esc_attr($_REQUEST["ux_txt_captcha_input"]))) {
         ...
         if (($userdata && wp_check_password($password, $userdata->user_pass)) || ($user_email_data && wp_check_password($password, $user_email_data->user_pass))) {
            /* Captcha was matched */
            $_SESSION["captch_booster_login"] = true;
            user_log_in_success($username, $ip_address);
            return $user;
         } else {
            $_SESSION["captch_booster_login"] = false;
         ...
         if ("" == esc_attr($_REQUEST["ux_txt_captcha_input"])) {
            ...
            $error->add("captcha_booster_error", "<strong>" . captcha_booster_logical_error . "</strong>: " . $error_data_array["for_invalid_captcha_error"]);
         }
         return $error;
      }
   } else {
...
function captcha_booster_lost_password_check() {
   global $captcha_booster_options, $wpdb;
   $error_data = $wpdb->get_var
       (
       $wpdb->prepare
           (
   ...
   if (isset($_REQUEST["ux_txt_captcha_input"]) && "" == esc_attr($_REQUEST["ux_txt_captcha_input"])) {
      $errors = new WP_Error("captcha_wrong", "<strong>" . captcha_booster_logical_error . "</strong>: " . $error_data_array["for_captcha_empty_error"]);
      return $errors;
   }
   ...
   if (0 == strcasecmp(trim(captcha_booster_decode($_REQUEST["captcha_booster_result"], $str_key, $_REQUEST["captcha_booster_time"])), esc_attr($_REQUEST["ux_txt_captcha_input"]))) {
      ...
      $errors = new WP_Error("captcha_wrong", "<strong>" . captcha_booster_logical_error . "</strong>: " . $error_data_array["for_invalid_captcha_error"]);
      return $errors;
   }
   return;
}
...
function captcha_booster_register_check($login, $email, $errors) {
   ...
   if (is_multisite()) {
      ...
      if (isset($_REQUEST["ux_txt_captcha_input"]) && "" == esc_attr($_REQUEST["ux_txt_captcha_input"])) {
         $errors->add("captcha_blank", "<strong>" . captcha_booster_logical_error . "</strong>: " . $error_data_array["for_captcha_empty_error"]);
         return $errors;
      }
...
function captcha_booster_display_captcha() {
   ...
   if ($captcha_array["captcha_type_text_logical"] == "logical_captcha" && $captcha_array["mathematical_operations"] == "arrange_order") {
      ...
      $str_arrange_expretion .= ($arrange_type[$rand_arrange_array] == "Ascending") ? captcha_booster_ascending_order : captcha_booster_descending_order;
      $str_arrange_expretion .= "<br>" . captcha_booster_seperate_numbers . "<span style='color:red'>*</span><br><br>";
      ...
      ?>
      <input type="hidden" name="captcha_booster_result" value="<?php echo $str = captcha_booster_encode($arr_convert, $str_key, $captcha_time); ?>" />
      <input type="hidden" name="captcha_booster_time" value="<?php echo $captcha_time; ?>" />
      ...
      $str_relational_expretion .= captcha_booster_arithemtic . " : <span style='color:red'>*</span><br>";
      $str_relational_expretion .= ($relation_op[$rand_relation_op] == "Smaller") ? captcha_booster_smaller_number : captcha_booster_larger_number;
      $str_relational_expretion .= $array_number[0] . " ";
      $str_relational_expretion .= captcha_booster_logical_or;
      ...
      /* Add hidden field with encoding result */
      ?>
      <input type="hidden" name="captcha_booster_result" value="<?php echo $str = captcha_booster_encode($array_number[2], $str_key, $captcha_time); ?>" />
      <input type="hidden" name="captcha_booster_time" value="<?php echo $captcha_time; ?>" />
      <input type="hidden" value="Version: <?php echo $captcha_plugin_info["Version"]; ?>" />
      <?php
      echo $str_relational_expretion;
   } else {
      ...
      /* String for display */
      $str_math_expretion = "";
      $str_math_expretion .= captcha_booster_arithemtic . " : <span style='color:red'>*</span> <br>";
      ...
      /* Add hidden field with encoding result */
      $str_math_expretion .= '<input type="hidden" name="captcha_booster_result" value="' . $str = captcha_booster_encode($array_math_expretion[2], $str_key, $captcha_time) . '" />
...
function captcha_booster_comment_form_check() {
   ...
   $str_key = $captcha_booster_options["captcha_key"]["key"];
   if (isset($_REQUEST["ux_txt_captcha_input"]) && "" == esc_attr($_REQUEST["ux_txt_captcha_input"])) {
      wp_die(captcha_booster_logical_error . ":&nbsp" . $error_data_array["for_captcha_empty_error"]);
   }
   if (isset($_REQUEST["captcha_booster_result"]) && isset($_REQUEST["captcha_booster_time"]) && isset($_REQUEST["ux_txt_captcha_input"])) {
      if (0 == strcasecmp(trim(captcha_booster_decode($_REQUEST["captcha_booster_result"], $str_key, $_REQUEST["captcha_booster_time"])), esc_attr($_REQUEST["ux_txt_captcha_input"]))) {
         return;
         /* Captcha was matched */
      } else {
         wp_die(captcha_booster_logical_error . ":&nbsp" . $error_data_array["for_invalid_captcha_error"]);
      }
   }
}
...
function captcha_booster_lost_password($user) {
   ...
   if (isset($_REQUEST["captcha_booster_result"]) && isset($_REQUEST["ux_txt_captcha_input"]) && isset($_REQUEST["captcha_booster_time"]) && 0 == strcasecmp(trim(captcha_booster_decode($_REQUEST["captcha_booster_result"], $str_key, $_REQUEST["captcha_booster_time"])), esc_attr($_REQUEST["ux_txt_captcha_input"]))) {
      return $user;
   } else {
...
function captcha_booster_encode($String, $Password, $captcha_time) {
   ...
   if (!$Password) {
      die(captcha_booster_encryption);
   }
...
function captcha_booster_decode($String_orignal, $Key, $captcha_time) {
   /* Check if key for encoding is empty */
   if (!$Key) {
      die(captcha_booster_decryption);
   }
   $Salt = md5($captcha_time, true);
